...
const osThreadAttr_t tsk_controller_attributes = { .name = "tsk_controller",
		.cb_mem = &controllerTaskControlBlock, .cb_size =
				sizeof(controllerTaskControlBlock), .stack_mem =
				&controllerTaskBuffer[0], .stack_size =
				sizeof(controllerTaskBuffer), .priority =
				(osPriority_t) osPriorityNormal, };
/* Definitions for tsk_usb */
osThreadId_t tsk_usbHandle;
uint32_t usbTaskBuffer[256];
osStaticThreadDef_t usbTaskControlBlock;
const osThreadAttr_t tsk_usb_attributes = { .name = "tsk_usb", .cb_mem =
...
const osThreadAttr_t tsk_manager_attributes = { .name = "tsk_manager", .cb_mem =
		&tsk_managerControlBlock, .cb_size = sizeof(tsk_managerControlBlock),
		.stack_mem = &tsk_managerBuffer[0], .stack_size =
				sizeof(tsk_managerBuffer), .priority =
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint16_t adc_dma_buffer[2];  // CH1, CH3
/* USER CODE END 0 */
...
int main(void) {
	...
	tsk_controllerHandle = osThreadNew(controllerTask, NULL,
			&tsk_controller_attributes);

	/* creation of tsk_usb */
	tsk_usbHandle = osThreadNew(usbTask, NULL, &tsk_usb_attributes);

	/* creation of tsk_manager */
	tsk_managerHandle = osThreadNew(mangerTask, NULL, &tsk_manager_attributes);
...
/**
 * @brief  Function implementing the tsk_controllerT thread.
 * @param  argument: Not used
 * @retval None
 */
/* USER CODE END Header_controllerTask */
__weak void controllerTask(void *argument) {
...
/* USER CODE BEGIN Header_mangerTask */
/**
 * @brief Function implementing the tsk_manager thread.
 * @param argument: Not used
 * @retval None
 */
/* USER CODE END Header_mangerTask */
__weak void mangerTask(void *argument) {
	/* USER CODE BEGIN mangerTask */
	/* Infinite loop */
	for (;;) {
		osDelay(1);
	}
	/* USER CODE END mangerTask */
}
