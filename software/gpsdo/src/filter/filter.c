...
static const float R = ((F_OSC_HZ / EXPECTED_CTR) * (F_OSC_HZ / EXPECTED_CTR)) / 12.0f;
...
static inline void mat_copy_f32(const arm_matrix_instance_f32 *src, arm_matrix_instance_f32 *dst) {
    memcpy(dst->pData, src->pData, src->numRows * src->numCols * sizeof(float32_t));
}
